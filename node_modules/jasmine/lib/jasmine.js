...
import { resolve as _resolve, isAbsolute, join, normalize } from 'path';
import { format } from 'util';
import { sync } from 'glob';
import Loader from './loader';
import ExitHandler from './exit_handler';
import ConsoleSpecFilter from './filters/console_spec_filter';
...
class Jasmine {
  constructor(options) {
    ...
    this.isWindows_ = (options.platform || platform)() === 'win32';
    ...
    if (options.projectBaseDir) {
      ...
      this.projectBaseDir = (options.getcwd || _resolve)();
    }
  ...
  async loadSpecificConfigFile_(relativePath) {
    const absolutePath = _resolve(this.projectBaseDir, relativePath);
...
function addFiles(kind) {
  return function (files) {
    ...
    const {includeFiles, excludeFiles} = files.reduce(function(ongoing, file) {
      ...
      if (!isAbsolute(file)) {
        file = join(jasmineRunner.projectBaseDir, jasmineRunner.specDir, file);
      }
    ...
    includeFiles.forEach(function(file) {
      const filePaths = sync(file, { ignore: excludeFiles })
        .filter(function(filePath) {
          ...
          return fileArr.indexOf(filePath) === -1 && fileArr.indexOf(normalize(filePath)) === -1;
        });
...
export const ConsoleReporter = require('./reporters/console_reporter');
